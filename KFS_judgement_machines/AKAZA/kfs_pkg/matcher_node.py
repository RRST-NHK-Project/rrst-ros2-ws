...
def detect_on_warped(warped_img, query_img, akaze):
    ...
    # 精度チェック
    good_w = [m for m, n in matches if m.distance < 0.8 * n.distance]
    return len(good_w)
        ...
        if des_query is not None:
            ...
            if len(good) > 1:
                ...
                def check_homography_quality(dst):
                    ...
                    if area < 5000: return False # 5000画素以下は無視
                    # 2. 凸性のチェック（四角形がねじれていないか）
                    if not cv2.isContourConvex(np.int32(dst)): return False
                    # 3. アスペクト比の確認（細長すぎたら誤検知）
                    # (辺の長さの比が極端でないか等をチェック)
                    return True

                # マッチング結果の描画
                result_img = cv2.drawMatchesKnn(query_img, kp_query, map_img, kp_map, good[:p_num], None, flags=0)

                # ホモグラフィ行列の計算
                if len(good) >= 4:
                    src_pts = np.float32([kp_query[m[0].queryIdx].pt for m in good]).reshape(-1, 1, 2)
                    dst_pts = np.float32([kp_map[m[0].trainIdx].pt for m in good]).reshape(-1, 1, 2)
                    ...
                    if M is not None:
                        ...
                        cv2.polylines(result_img, [np.int32(dst_for_draw)], True, (0, 0, 255), 10, cv2.LINE_AA)

                # 投票システムによる座標特定
                query_kp_list = [kp_query[p[0].queryIdx] for p in good[:p_num]]
                map_kp_list = [kp_map[p[0].trainIdx] for p in good[:p_num]]
                deg_value, size_rate, m1, m2 = vote_point(query_kp_list, map_kp_list, p_num)

                if deg_value is not None:
                    # 中心点計算
                    q_x1, q_y1 = query_kp_list[m1].pt # クエリ側の基準座標
                    m_x1, m_y1 = map_kp_list[m1].pt # マップ側の基準座標
                    q_xcenter, q_ycenter = int(width_query/2), int(height_query/2) # クエリ画像の中心座標
                    q_center_deg = math.atan2(q_ycenter - q_y1, q_xcenter - q_x1) * 180 / math.pi # 基準点から中心点への角度
                    q_center_len = math.sqrt((q_xcenter - q_x1) ** 2 + (q_ycenter - q_y1) ** 2) # 基準点から中心点への距離
