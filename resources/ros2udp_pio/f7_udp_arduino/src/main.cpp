#include <STM32Ethernet.h>  // STM32Ethernetライブラリをインクルード
#include <EthernetUdp.h>     // UDP通信を管理するライブラリ
#include <vector>            // std::vectorを使用するためにインクルード

// MACアドレスと静的IPアドレスの設定
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED };  // 任意のMACアドレス
IPAddress ip(192, 168, 0, 215);  // 静的IPアドレス
