...
## 配列要素について
現時点ではROS2ノードからマイコンに送信される配列dataは９個の要素を持っています。0番は未使用で送信もされません。NHKロボコン2025では電磁弁のリレー駆動、サーボモーターの駆動、状態表示のために配列要素の拡張(9番以降)を予定しています。追加予定の要素も含めた各要素の詳細をここにまとめます。
| data[n] | 詳細 |
| ---- | ---- |
