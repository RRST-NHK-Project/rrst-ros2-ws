...
だけの機能)では正しく較正できない。そこで本ノードは:
  ...
  4. センサ検出時点の生ENC値から r_raw を計算し、r_offset_m = r_ref_value_m -
     r_raw を求める
  5. z_offset_m/r_offset_mをreal_joint_bridge_nodeへSetParametersサービスで
     反映する(real_joint_bridge_node側もyamlではなくランタイムパラメータとして
     これを保持する設計。詳細はreal_joint_bridge_node.py冒頭コメント参照)
...
class HomingNode(Node):
    ...
    def _apply_offsets(self, z_offset_m, r_offset_m):
        if not self._set_params_cli.service_is_ready():
            self.get_logger().error(
                'homing_node: bridge node set_parameters service not available, '
                'offsets NOT applied. Is real_joint_bridge_node running?')
            return
        params = [
            Parameter(name='z_offset_m',
                      value=ParameterValue(type=ParameterType.PARAMETER_DOUBLE,
                                            double_value=float(z_offset_m))),
            Parameter(name='r_offset_m',
                      value=ParameterValue(type=ParameterType.PARAMETER_DOUBLE,
                                            double_value=float(r_offset_m))),
        ]
        future = self._set_params_cli.call_async(SetParameters.Request(parameters=params))

        def _done(fut):
            try:
                fut.result()
                self.get_logger().info('homing_node: offsets applied to real_joint_bridge_node')
            except Exception as exc:
                self.get_logger().error(f'homing_node: failed to apply offsets: {exc}')
