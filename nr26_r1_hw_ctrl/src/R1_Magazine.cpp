...
public:
        ...
        : Node("hardware_control_" + std::to_string(OUTPUT_DEVICE_ID))
    {
        ...
        */
        ...
        // GUI -> magazine action subscriber
        magazine_action_sub_ = this->create_subscription<std_msgs::msg::Int32MultiArray>(
            "r1_magazine_action", 10,
            std::bind(&HardWareControl::onMagazineAction, this, std::placeholders::_1));

        // status text publisher for human-readable feedback
        status_text_pub_ = this->create_publisher<std_msgs::msg::String>(
            "r1/task_status_text", rclcpp::QoS(1).reliable().transient_local());
...
private:
    ...
    void onMagazineAction(const std_msgs::msg::Int32MultiArray::SharedPtr msg)
    {
        ...
        int32_t slot = static_cast<int32_t>(data[0]);
        int32_t action = static_cast<int32_t>(data[1]);
        ...
        static const int MAG_SERVO_ANGLE[] = {270, 233, 187, 139, 99, 55, 4};
        static const int BAR_HOLD_ANGLE = 43;
        static const int BAR_PUSH_ANGLE = 10;
        static const int BAR_RELE_ANGLE = 245;
        static const int EJECT_ANGLE = 90;

        // Set data_[9] according to user-specified mapping:
        // MAG_SERVO_ANGLE = {270,233,187,139,99,55,4}
        // Slot1: collect=270, use=MAG[3], discard=MAG[4]
        // Slot2: collect=MAG[1], use=MAG[2], discard=MAG[5]
        // Slot3: collect=MAG[2], use=MAG[5], discard -> fallback MAG[5]
        // Slot4: collect=MAG[3], use=MAG[6], discard=MAG[6]
        switch (slot) {
            case 1:
                if (action == 0) data_[9] = MAG_SERVO_ANGLE[0];
                else if (action == 1) data_[9] = MAG_SERVO_ANGLE[3];
                else if (action == 2) data_[9] = MAG_SERVO_ANGLE[4];
                else data_[9] = MAG_SERVO_ANGLE[0];
                break;
            case 2:
                if (action == 0) data_[9] = MAG_SERVO_ANGLE[1];
                else if (action == 1) data_[9] = MAG_SERVO_ANGLE[2];
                else if (action == 2) data_[9] = MAG_SERVO_ANGLE[5];
                else data_[9] = MAG_SERVO_ANGLE[1];
                break;
            case 3:
                if (action == 0) data_[9] = MAG_SERVO_ANGLE[2];
                else if (action == 1) data_[9] = MAG_SERVO_ANGLE[5];
                else if (action == 2) data_[9] = MAG_SERVO_ANGLE[5];
                else data_[9] = MAG_SERVO_ANGLE[2];
                break;
            case 4:
                if (action == 0) data_[9] = MAG_SERVO_ANGLE[3];
                else if (action == 1) data_[9] = MAG_SERVO_ANGLE[6];
                else if (action == 2) data_[9] = MAG_SERVO_ANGLE[6];
                else data_[9] = MAG_SERVO_ANGLE[3];
                break;
            default:
        ...
        // action: 0=collect(回収), 1=use(使用), 2=discard(廃棄),
        //         3=lock(ロック), 4=unlock(ロック解除),
        //         5=load(装填), 6=unload(装填解除)
        if (action == 0) {
            // collect: prepare and hold
            data_[10] = BAR_HOLD_ANGLE;
        } else if (action == 1) {
            // use: push
            data_[10] = BAR_PUSH_ANGLE;
            ...
            // unlock: release to home position
            data_[10] = BAR_RELE_ANGLE;
        } else if (action == 5) {
            // load: push
            data_[10] = BAR_PUSH_ANGLE;
        ...
        if (status_text_pub_) {
            ...
            const char *act = (action == 0) ? "回収" :
                              ...
                              (action == 5) ? "装填" :
                              (action == 6) ? "装填解除" : "不明";
            oss << "Hardware: Magazine action received slot=" << slot << " action=" << act;
            auto out = std::make_shared<std_msgs::msg::String>();
            out->data = oss.str();
            status_text_pub_->publish(*out);
        }

        RCLCPP_INFO(get_logger(), "Applied magazine action: slot=%ld action=%ld", static_cast<long>(slot), static_cast<long>(action));
    }
