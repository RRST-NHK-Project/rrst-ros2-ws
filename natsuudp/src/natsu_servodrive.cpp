...
public:
    PS4_Listener(const std::string &ip, int port)
        ...
        RCLCPP_INFO(this->get_logger(),
                    "NATSUROBO2025 SD initialized with IP: %s, Port: %d", ip.c_str(),
                    port);
    }
...
public:
    Servo_Deg_Publisher() : Node("natsu_servo_deg_publisher") {
        // Publisherの作成
...
int main(int argc, char *argv[]) {
    ...
    // figletでノード名を表示
    std::string figletout = "figlet Natsu SwerveDrive";
    ...
    auto ps4_listener = std::make_shared<PS4_Listener>(IP_NATSU_SD, PORT_NATSU_SD);
