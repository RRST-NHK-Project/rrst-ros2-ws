...
class ArucoRealSenseDetector(Node):
    """RealSense映像トピックからArUcoを検出して3D姿勢を配信するノード."""

    def __init__(self):
        super().__init__("aruco_realsense_detector")
        ...
        self.overlay_pub = self.create_publisher(Image, "/aruco/image_annotated", 10)
        ...
        self.get_logger().info(
            "RealSense detector started: "
            f"input_image_topic={self.input_image_topic}, pose_topic={self.pose_topic}"
        )
    ...
    def _image_callback(self, msg: Image):
        ...
        if ids is not None and len(corners) > 0:
            ...
            self._publish_pose_and_distance(tvec, rvec, msg.header)
    ...
    def _publish_pose_and_distance(self, tvec, rvec, header):
...
def main(args=None):
    ...
    node = None
    try:
        node = ArucoRealSenseDetector()
        rclpy.spin(node)
    except Exception as exc:
        if node is not None:
            node.get_logger().error(str(exc))
        else:
            print(f"[aruco_realsense_detector] {exc}")
    finally:
        if node is not None:
            node.destroy_node()
        rclpy.shutdown()
