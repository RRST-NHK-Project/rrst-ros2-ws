...
## 概要 
4輪オムニ試作機向けに開発したパッケージです。キャチロボ2024でも採用、NHKロボコン2025ではC++移植版のf7_udp_cppパッケージを採用予定です。
...
フィードバック制御に対応するためにエンコーダーから速度(m/s)を求め、UDPで返す機能があります。（1番と２番のエンコーダーのみ動作確認済み）
...
## 配列要素について
ROS 2ノードからマイコンに送信される配列'data'は9個の要素を持っています。各要素の詳細をここにまとめます。モーター以外の駆動には未使用の要素を転用しています。
| data[n] | 詳細 |
| ---- | ---- |
